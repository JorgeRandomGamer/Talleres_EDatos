            ...
            if(!columnaLlena(col)){
                for(int fila = filas - 1; fila >= 0; fila--){
                    if(tablero[fila][col] == 0){
                        ...
                        int puntaje = minimax(1, 2, INT_MIN, INT_MAX);
                        tablero[fila][col] = 0;
...
int main(){
    ...
    for(int i = 0; i < filas * columnas; i++){
        ...
        if(jugadorActual == 1){
            int col;
            cout<<"Ingrese la columna (1-7): ";cin>>col;

            while (!juego.hacerMovimiento(col-1, jugadorActual)) {
                cout<<"Movimiento no válido. Ingrese la columna nuevamente: ";cin>>col;
            }
        }else{juego.jugar();}
